...
export default function UserPhotos() {
    ...
    const fetchUserPhotos = async () => {
      try {
        const response = await fetch(
          `https://ngl6xs-3001.csb.app/photos/user/${userId}`,
          {
            credentials: "include",
          }
        );
        ...
        data.photos.sort((a, b) => new Date(b.uploadTime) - new Date(a.uploadTime));
        setUserData(data);
      } catch (err) {
  ...
  const handleCommentSubmit = async (photoId) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://ngl6xs-3001.csb.app/commentsOfPhoto/${photoId}`,
        {
          method: "POST",
          headers: {
          ...
          {userData.photos.map((photo) => (
            <div key={photo._id} style={photoCardStyle}>
              <div style={imageContainerStyle}>
                <img src={photo.url} alt="User" style={imageStyle} />
              </div>
              <div style={postedTimeStyle}>
                Posted on: {new Date(photo.uploadTime).toLocaleString()}
              </div>
